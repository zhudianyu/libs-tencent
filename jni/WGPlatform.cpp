...
void WGPlatform::WGSetQQGroupObserver(WGQQGroupObserver* pQQGroupNotify) {
	LOGD("WGPlatform::WGSetQQGroupObserver%s", "");

	if (pQQGroupNotify == NULL) {
		LOGI("pQQGroupNotify is NULL%s", "");
		return;
	}
	mQQGroupObserver = pQQGroupNotify;
}
...
WGQQGroupObserver* WGPlatform::GetQQGroupObserver() const {
	return mQQGroupObserver;
}
...
void WGPlatform::WGUnbindQQGroup(unsigned char* cGroupOpenid,unsigned char* cUnionid){
	...
	jmethodID jWGUnbindQQGroup =
				env->GetStaticMethodID(s_WGPlatformClass,
						...
						"(Ljava/lang/String;Ljava/lang/String;)V");
	jstring jUnionid = env->NewStringUTF((char const *) cUnionid);
	jstring jGroupOpenid = env->NewStringUTF((char const *) cGroupOpenid);
 ...
 * @param scene 公告场景ID
 */
std::vector<NoticeInfo> WGPlatform::WGGetNoticeData(unsigned char* cScene){
	JNIEnv *env;
	m_pVM->AttachCurrentThread(&env, NULL);
	...
	jNoticeVectorObj = env->CallStaticObjectMethod(s_WGPlatformClass,jWGGetNoticeMethod,jScene);
	//noticeinfo 对象
